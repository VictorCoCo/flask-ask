...
class SmokeTestUsingSamples(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.env = {"PYTHONPATH": project_root, "ASK_VERIFY_REQUESTS": "false"}
        if os.name == "nt":
    ...
    def _launch(self, sample):
        ...
        time.sleep(1)
        self.assertIsNone(
            process.poll(), msg="Poll should work," "otherwise we failed to launch"
        )
    ...
    def tearDown(self):
        try:
            ...
            try:
                ...
                print(
                    "*** it is recommended you manually kill with PID %s",
                    self.process.pid,
                )
