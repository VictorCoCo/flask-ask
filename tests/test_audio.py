...
class AudioUnitTests(unittest.TestCase):
    def setUp(self):
        ...
        self.context_patcher = patch(
            "flask_ask.models.context", return_value=MagicMock()
        )
    ...
    def test_custom_token(self):
        ...
        token = "hello_world"
        audio_item = audio()._audio_item(
            stream_url="https://fakestream", offset=10, opaque_token=token
        )
    ...
    def test_setting_and_getting_current_stream(self):
        ...
        with patch("flask_ask.core.find_ask", return_value=ask):
            ...
            stream.__dict__.update(
                {"token": "asdf", "offsetInMilliseconds": 123, "url": "junk"}
            )
